...
make_timeseries_svg = function(dates, values, width = 320L, height = 200L) {
  ml = 44; mr = 12; mt = 12; mb = 52
  ...
  for (t in ticks) {
    ...
    grid = paste0(grid,
      sprintf('<line x1="%f" y1="%f" x2="%f" y2="%f" stroke="#e6e6e6" stroke-width="1"/>',
              ml, yy, ml + pw, yy),
      sprintf('<text x="%f" y="%f" font-size="9" fill="#555" text-anchor="end" font-family="sans-serif">%s</text>',
              ml - 5, yy + 3, format(t, big.mark = ",")))
  }
  ...
  for (i in seq_len(n)) {
    ...
    xlab = paste0(xlab, sprintf(
      '<text x="%f" y="%f" font-size="9" fill="#555" text-anchor="end" font-family="sans-serif" transform="rotate(-40 %f %f)">%s</text>',
      cx, mt + ph + 12, cx, mt + ph + 12, format(dates[i], "%b %Y")))
  }

  ytitle = sprintf(
    '<text x="11" y="%f" font-size="9" fill="#555" text-anchor="middle" font-family="sans-serif" transform="rotate(-90 11 %f)">Bats detected</text>',
    mt + ph / 2, mt + ph / 2)
...
There is a growing need to understand bat activity in offshore environments as the exploration and development of offshore wind energy facilities expands. Several bat species are known to use offshore routes during migration and movement, including three Canadian migratory species, *Lasiurus cinereus*, *Lasiurus borealis*, and *Lasionycteris noctivagans*, which have been assessed as Endangered by COSEWIC [@solick2021; @canada2024]. To better understand bat use of offshore environments and assess the potential impacts of offshore wind energy facilities, the Canadian Wildlife Service is monitoring bats offshore using both stationary and mobile acoustic detectors, following the guidelines set forth by the Regional Wildlife Science Collaborative for Offshore Wind [@integrat]

![Silver-haired bat (*Lasyonicteris noctivagans*). Photo by Jason Headley](Icons/LANO-JasonHeadley.jpg)

## Methods

The Canadian Wildlife Service deployed acoustic units to record bats at 13 stationary sites along the maritime coastline, and 4 mobile transects on 4 different vessels (Cartier, DFO-Discovery, NRCAN-Discovery and Coriolis), with the NRCAN-Discovery vessel carried 3 microphones positioned on different sections of the ship (main mast, met mast A, and met mast B). Sampling occurred mainly in the summer and fall of 2025, with additional sampling from 2024 and 2026 submitted for review.
...
In total, 226,660 recordings were submitted to SENSR for review. Of these, 204,416 were identified as Noise by Kaleidoscope, and the remaining 22,244 were manually reviewed and assigned a species or species-grouping ID. Manual verification flagged an additional 1,910 recordings as noise that Kaleidoscope had erroneously identified as bats, leaving a total of 20,334 bat recordings across all sites. Of these, 14 were recorded along the mobile ship-based routes, while the remainder came from stationary acoustic recorders deployed along the coastline ([@fig-SiteSummary]).
